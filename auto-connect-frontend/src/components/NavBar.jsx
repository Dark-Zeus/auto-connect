...
function NavBar({ navLinks, version, icon, userId }) {
  ...
  const navLinkElements = navLinks.map((link, index) => {
    ...
    return (
      <li
        key={index}
        className={`nav-link ${isNavLinkActive(link) ? "selected" : ""}`}
        title={link.title}
      >
        <label htmlFor={index} className="link-label">
          <span className="nav-link__main">
            <ContainerType
              ...
              className={`nav-link__details nav-link ${
                isNavLinkActive(link) ? "selected" : ""
              }`}
            >
              <span className="nav-link__icon material-symbols-outlined-fill">
                {" "}
                {link.icon}
              </span>
            ...
            {link.sub?.map((subLink, subIndex) => (
              ...
              >
                ...
                >
                  ...
                  <span className="nav-link__text">{subLink.title}</span>
                </Link>
              </li>
            ))}
          </ul>
  ...
  return (
    <div className="nav">
      ...
      <div className="nav__wrapper">
        <ul className="nav-links">{navLinkElements}</ul>
      </div>
      <div className="nav-container">
