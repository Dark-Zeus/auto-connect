      ...
      if (i < fullStars) {
        stars.push(
          <Star
            key={i}
            size={16}
            className="tw:text-yellow-400 tw:fill-current"
          />
        );
      ...
      {services.length > 0 && (
        <div className="tw:flex tw:flex-wrap tw:gap-2 tw:mb-4">
          {services.slice(0, 3).map((service, index) => (
            <span 
              key={index}
              className="tw:text-xs tw:bg-blue-50 tw:text-blue-700 tw:px-2 tw:py-1 tw:rounded-full tw:font-medium"
            >
              {service}
            </span>
          ))}
          {services.length > 3 && (
            <span className="tw:text-xs tw:text-gray-500 tw:px-2 tw:py-1">
