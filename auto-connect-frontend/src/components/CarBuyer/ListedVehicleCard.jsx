...
const ListedVehicleCard = ({ vehicle }) => {
  ...
  const formatOdometer = (mileage) =>
    mileage != null ? `${Number(mileage).toLocaleString()} km` : "N/A";
  ...
  const postedTimestamp =
    vehicle?.promotion === 1 && vehicle?.bumpSchedule?.lastBumpTime
      ...
      : vehicle?.effectiveCreatedAt ?? vehicle?.createdAt;
    ...
    >
      <Box sx={{ display: "flex", width: "100%", height: "100%" }}>
        ...
        <Box sx={{ width: "70%", height: "100%" }}>
          <CardContent
            className="tw:p-4 tw:h-full tw:flex tw:flex-col"
            sx={{ height: "100%", boxSizing: "border-box", padding: "16px !important" }}
          >
            <Box onClick={handleHeaderClick} className="tw:cursor-pointer tw:mb-3 tw:group">
              <Typography
                variant="h6"
                component="h2"
            ...
            <Box className="tw:space-y-2 tw:flex-grow">
              <Box className="tw:flex tw:items-center tw:gap-2">
                <LocationOn fontSize="small" style={{ color: "#7bb1d2" }} />
                <Typography variant="body2" className="tw:text-gray-700" fontSize={14}>
                  {vehicle?.district}, {vehicle?.city}
                </Typography>
              </Box>

              <Box className="tw:flex tw:items-center tw:gap-2">
                <AttachMoney fontSize="small" style={{ color: "#7bb1d2" }} />
                <Typography variant="h6" className="tw:font-bold" style={{ color: "#4a618a" }}>
                  {formatPrice(vehicle?.price)}
                </Typography>
              </Box>

              <Box className="tw:flex tw:items-center tw:gap-2">
                <Speed fontSize="small" style={{ color: "#7bb1d2" }} />
                <Typography variant="body2" className="tw:text-gray-700" fontSize={13} fontWeight={600}>
                  {formatOdometer(vehicle?.mileage)}
                </Typography>
              </Box>

              <Box className="tw:flex tw:items-center tw:gap-2">
                <LocalGasStation fontSize="small" style={{ color: "#7bb1d2" }} />
                <Chip
            ...
            <Box className="tw:flex tw:justify-end tw:mt-3 tw:pt-2 tw:border-t tw:border-gray-300">
              <Typography variant="caption" className="tw:text-gray-500 tw:text-xs" fontSize={12}>
                Posted: {formatDate(postedTimestamp)}
              </Typography>
            </Box>
          </CardContent>
        </Box>
